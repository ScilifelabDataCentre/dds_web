...
from dds_web import app_obj, db, C_TZ
...
def invoice_units():
    ...
    app_obj.logger.debug("Calculating invoicing info...")
                ...
                for p in f.projects:
                    ...
                    for v in p.file_versions:
                        ...
                        if v.time_deleted and v.time_invoiced and v.time_deleted == v.time_invoiced:
                            app_obj.logger.debug(f"Period invoiced fully : {v}")
                            continue

                        start, end = ("", "")
                        if not v.time_invoiced:  # not included in invoice
                            app_obj.logger.debug(f"Invoice = NULL : {v}")
...
def remove_invoiced():
    ...
    app_obj.logger.debug("Removing deleted and invoiced versions...")
            ...
            for v in all_versions:
                ...
                if v.time_deleted and v.time_invoiced and v.time_deleted == v.time_invoiced:
                    ...
                    if diff.seconds > 60:  # TODO (ina): Change to correct interval -- 30 days?
                        app_obj.logger.debug(f"Deleting: {v}")
                        db.session.delete(v)
                        db.session.commit()
...
def remove_expired():
    ...
    app_obj.logger.debug("Cleaning up File table...")
    ...
    with app_obj.app_context():
        try:
            ...
            for file in page_query(
                ...
                app_obj.logger.debug("File: %s - Expires: %s", file, file.expires)
            ...
            # TODO (ina, senthil): Something else should happen here
            app_obj.logger.warning(f"test: {err}")
...
scheduler = BackgroundScheduler(
    {
        "apscheduler.jobstores.default": {
            "type": "sqlalchemy",
            # "url": app_obj.config.get("SQLALCHEMY_DATABASE_URI"),
            "engine": db.engine,
        },
