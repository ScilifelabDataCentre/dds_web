...
def create_app(testing=False, database_uri=None):
    try:
        ...
        with app.app_context():  # Everything in here has access to sessions
            ...
            # Redis Worker needs to run as its own process, we initialize it here. There will be a worker for each instance of the app running
            redis_url = app.config.get("REDIS_URL")
            redis_connection = Redis.from_url(redis_url)
            hostmame = socket.gethostname()

            # Flask calls the create_app function twice during startup. If the
            # redis worker has already been started, we don't want to start it again.
            rq_name = f"rq:worker:{hostmame}:"  # prefix generated by rq

            if not Worker.find_by_key(worker_key=rq_name, connection=redis_connection):
