...
def test_new_file_put_does_not_update_project_timestamp(client):
    """PUT /file/new must not update project.date_updated or last_updated_by."""
    import freezegun
    import datetime
...
def test_match_file_endpoint(client):
    ...
    query_files = {"files": ["non_existent_file"]}
    response = client.get(
        tests.DDSEndpoint.FILE_MATCH,
        headers=tests.UserAuth(tests.USER_CREDENTIALS["unitadmin"]).token(client),
        query_string={**query_files, "project": "file_testing_project"},
