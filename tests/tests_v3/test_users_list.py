...
def test_list_unitusers_with_super_admin_nonexistent_unit(client):
    ...
    response = client.get(
        tests.DDSEndpoint.LIST_USERS, query_string={"unit": incorrect_unit}, headers=token
    )
...
def test_list_unitusers_with_super_admin_correct_unit(client):
    ...
    token = get_token(username=users["Super Admin"], client=client)
    response = client.get(
        tests.DDSEndpoint.LIST_USERS, query_string={"unit": unit_row.public_id}, headers=token
    )
    assert response.status_code == http.HTTPStatus.OK
